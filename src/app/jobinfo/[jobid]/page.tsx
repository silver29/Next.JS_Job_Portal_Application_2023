...
function Jobinfo() {
  ...
  const onApply = async () => {
    try {
      dispatch(SetLoading(true));
      //const response = await axios.post(`/api/jobs/applications`, { -- 405 Method Not Allowed
  ...
  return (
    // TypeError: Cannot read properties of null (reading 'title') "jobData &&"
    jobData && (
      <div>
        ...
        <Row gutter={[16, 16]} className="gap-3">
          <Col span={12} className="flex flex-col gap-2">
            ...
            <div className="flex justify-between">
              <span>Salary</span>
              <span>{jobData.salaryFromRange} LPA - {jobData.salaryToRange} LPA</span>
            </div>
            <div className="flex justify-between">
              <span>Work Mode</span>
